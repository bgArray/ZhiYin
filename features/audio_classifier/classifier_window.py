...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QProgressBar, QGroupBox, QTextEdit, QLineEdit
)
...
class AudioClassifierWindow(BaseWindow):
    ...
    def _create_results_widget(self):
        ...
        self.canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)  # 允许扩展
        ...
        self.figure.subplots_adjust(bottom=0.15)  # 增加底部边距，确保"时间(秒)"标签不被遮挡
        ...
        self.legend_content.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)  # 允许扩展
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.model_status_action.setText(f"模型状态: 已加载 ({device_str}, {len(self.label_names)}标签)")
    ...
    def _on_processing_complete(self, y, sr, mel_spectrogram_db, prob_matrix, time_stamps, duration):
        """处理完成回调"""
        if self.progress_dialog:
    ...
    def _recognize_lyrics(self):
        ...
        self.lyrics_thread = LyricsRecognizerThread(
            self.current_file, 
            preserve_pronunciation=preserve_pronunciation
        )
    ...
    def _on_lyrics_complete(self, lyrics_data):
        ...
        if "text" in lyrics_data:
            ...
            if lyrics_data.get("preserve_pronunciation", False) and "original_text" in lyrics_data:
            ...
            try:
                ...
                if isinstance(self.aligned_result, dict):
                    ...
                    if "aligned_words" in self.aligned_result:
                        print(f"对齐词数量: {len(self.aligned_result['aligned_words'])}")
                ...
                QMessageBox.information(self, "保存成功", f"对齐结果已保存到:\n{file_path}")
    ...
    def _update_visualization(self):
        ...
        for i, label_name in enumerate(self.label_names):
            ...
            # 检查该标签是否被选中
            if label_name not in self.label_checkboxes or not self.label_checkboxes[label_name].isChecked():
                continue  # 跳过未选中的标签
            ...
            # 绘制概率曲线
            ax.plot(time_stamps, probs, label=label_name, color=color, linewidth=2, alpha=0.9)
            
            # 绘制阈值线下方的区域
            ax.fill_between(time_stamps, 0, probs, where=(probs >= self.threshold), 
        ...
        # 绘制阈值线
        ax.axhline(y=self.threshold, color='r', linestyle='--', alpha=0.7, linewidth=1.5, label=f'阈值: {self.threshold:.2f}')
    ...
    def _update_playback(self):
        ...
        # 获取当前播放位置
        current_pos = self.waveform_widget.current_playback_sample / self.waveform_widget.sample_rate
    ...
    def _update_timeline_manager(self, new_position=None, from_progress_bar=False):
        ...
        if not from_progress_bar and not is_playhead_visible and self.is_playing:
            ...
            if new_position < start_sec:
                ...
                # 计算新的偏移量
                self.waveform_widget.offset_x = -new_start * self.waveform_widget.zoom_level
        ...
        if not from_progress_bar:
            # 计算绝对时间进度（0-1000）
            if self.waveform_widget.total_seconds > 0:
                absolute_progress = (new_position / self.waveform_widget.total_seconds) * 1000
    ...
    def _update_progress_bar_for_zoom(self):
        ...
        if self.waveform_widget.playhead_position < start_sec:
            ...
            relative_pos = ((self.waveform_widget.playhead_position - start_sec) / visible_range) * 1000
